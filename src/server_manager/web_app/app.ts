...
type DigitalOceanServerRepositoryFactory = (session: digitalocean_api.DigitalOceanSession) =>
    DigitaloceanServerRepository;

export class App {
  private digitalOceanRepository: DigitaloceanServerRepository;
  ...
  private async enterDigitalOceanMode(accessToken: string): Promise<server.ManagedServer[]> {
    ...
    this.digitalOceanRepository.registerApiConnectionIssueListener(async () => {
      const shouldDisconnect = await this.appRoot.showConnectivityDialog();
      if (shouldDisconnect) {
        this.clearCredentialsAndShowIntro();
      }
    });
    ...
    try {
      const account = await this.digitalOceanRepository.getAccount();
    ...
    return await this.digitalOceanRepository.listServers();
  }
  ...
  private async showServer(selectedServer: server.Server, selectedDisplayServer: DisplayServer) {
    ...
    try {
      ...
      this.notificationManager.showError('error-keys-get');
    }
  ...
  private addAccessKey() {
    this.selectedServer.addAccessKey()
        .then((serverAccessKey: server.AccessKey) => {
          ...
          this.notificationManager.showNotification('notification-key-added');
        })
        .catch((error) => {
          console.error(`Failed to add access key: ${error}`);
          this.notificationManager.showError('error-key-add');
        });
  }
  ...
  private renameAccessKey(accessKeyId: string, newName: string, entry: polymer.Base) {
    this.selectedServer.renameAccessKey(accessKeyId, newName)
        ...
        .catch((error) => {
          ...
          this.notificationManager.showError('error-key-rename');
          entry.revertName();
        });
  }
  ...
  private async setAccessKeyDataLimit(limit: server.DataLimit) {
    ...
    try {
      ...
      this.notificationManager.showNotification('saved');
      ...
      this.notificationManager.showError('error-set-data-limit');
  ...
  private async removeAccessKeyDataLimit() {
    ...
    try {
      ...
      this.notificationManager.showError('error-remove-data-limit');
  ...
  private async setHostnameForAccessKeys(hostname: string, serverSettings: polymer.Base) {
    this.notificationManager.showNotification('saving');
    try {
      await this.selectedServer.setHostnameForAccessKeys(hostname);
      this.notificationManager.showNotification('saved');
      serverSettings.enterSavedState();
    } catch (error) {
      this.notificationManager.showError('error-not-saved');
  ...
  private removeAccessKey(accessKeyId: string) {
    this.selectedServer.removeAccessKey(accessKeyId)
        .then(() => {
          ...
          this.notificationManager.showNotification('notification-key-removed');
        })
        .catch((error) => {
          console.error(`Failed to remove access key: ${error}`);
          this.notificationManager.showError('error-key-remove');
        });
  }
  ...
  private async setMetricsEnabled(metricsEnabled: boolean) {
    ...
    try {
      ...
      this.notificationManager.showError('error-metrics');
      serverView.metricsEnabled = !metricsEnabled;
    }
  }
  ...
  private async renameServer(newName: string) {
    ...
    try {
      ...
      this.notificationManager.showError('error-server-rename');
