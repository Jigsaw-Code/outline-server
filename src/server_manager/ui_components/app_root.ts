...
declare namespace OutlineI18n {
  function getBestMatchingLanguage(languages: string[]): string;
}
...
export class AppRoot extends mixinBehaviors([AppLocalizeBehavior], PolymerElement) {
  ...
  @property({type: Object}) LANGUAGES_AVAILABLE = {
    am: {id: 'am', dir: 'ltr'},
    ar: {id: 'ar', dir: 'rtl'},
    bg: {id: 'bg', dir: 'ltr'},
    ca: {id: 'ca', dir: 'ltr'},
  ...
  } as {[key:string]: {id: string, dir: string}};

  @property({type: String}) DEFAULT_LANGUAGE = 'en';
  @property({type: Boolean}) useKeyIfMissing = true;
  @property({type: Array}) serverList = [] as DisplayServer[];
  @property({type: Object}) selectedServer = undefined as DisplayServer;
  @property({type: String}) adminEmail = '';
  @property({type: String}) outlineVersion: string;
  @property({type: String}) currentPage = 'intro';
  @property({type: Boolean}) shouldShowSideBar = false;
  @property({type: Boolean, observer: AppRoot.prototype._userAcceptedTosChanged}) userAcceptedTos: boolean;

  @computed('LANGUAGES_AVAILABLE', 'DEFAULT_LANGUAGE')
  get language(): string {
    return OutlineI18n.getBestMatchingLanguage(Object.keys(this.LANGUAGES_AVAILABLE)) ||
        this.DEFAULT_LANGUAGE;
  }
  @computed('serverList.*')
  get hasManualServers(): boolean {
    return this.serverList.filter((server: DisplayServer) => !server.isManaged).length > 0;
  }
  @computed('adminEmail')
  get isSignedInToDigitalOcean(): boolean {
    return Boolean(this.adminEmail);
  }
  @computed('userAcceptedTos')
  get hasAcceptedTos(): boolean {
    return this.userAcceptedTos || !!window.localStorage[TOS_ACK_LOCAL_STORAGE_KEY];
  }
  @computed('shouldShowSideBar')
  get sideBarMarginClass(): string {
    return this.shouldShowSideBar ? 'side-bar-margin' : '';
  }
  ...
  handleRegionSelected(e: Event) {
    this.fire('SetUpServerRequested', {regionId: this.$.regionPicker.getSelectedRegionId()});
  }
  ...
  showToast(message: string, duration: number) {
    ...
    setTimeout(() => {
      toast.show({text: message, duration, noOverlap: true});
    }, 0);
  }
  closeError() {
    this.$.toast.close();
  ...
  getConfirmation(
      title: string, text: string, confirmButtonText: string, continueFunc: () => void) {
  ...
  private _userAcceptedTosChanged(oldValue: boolean, newValue: boolean) {
    if (newValue) {
