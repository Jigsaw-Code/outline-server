...
export function runOauth(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        ...
        server.listen(0, 'localhost', () => {
            ...
            const targetUrl = `http://localhost:${encodeURIComponent(address.port.toString())}?secret=${encodeURIComponent(secret)}`;
            const oauthUrl = `https://cloud.digitalocean.com/v1/oauth/authorize?client_id=f744f93b192f6b5280129db478897111984181f45b3d42afd7a159c786899825&response_type=token&scope=read%20write&redirect_uri=https://fortuna.users.x20web.corp.google.com/jigsaw/outline/post_oauth.html&state=${encodeURIComponent(targetUrl)}`;
