...
(
  ...
  client_curl --insecure ${SB_API_URL}/server | grep -q 'accessKeyDataLimit' || echo 'Default data limit not set'
 ...
 # Verify that we can remove default data limits
  ...
  client_curl --insecure ${SB_API_URL}/server | grep -vq 'accessKeyDataLimit' || echo 'Default data limit not removed'
